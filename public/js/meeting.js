...
let userRole = sessionStorage.getItem('userRole') || 'student';
...
async function performSearch() {
    ...
    if (query.length < 2) {
        ...
        statusDiv.style.background = 'rgba(239, 68, 0.2)';
    ...
    try {
        ...
        students.forEach(student => {
            ...
            item.style.cssText = 'display: flex; justify-content: space-between; align-items: center; padding: 12px; background: rgba(255,255,0.05); border-radius: 8px; border: 1px solid rgba(25,255,0.1);';
...
const isLocalhost = hostname === 'localhost' || hostname === '127.0.1';
...
async function joinRoom(id) {
    try {
        ...
        // Handle name input based on role (use global userRole which was updated by initializeMeeting)
        let currentUserName = '';

        if (userRole === 'student') {
            currentUserName = userName;  // Use global userName
        } else {
            const nameInput = document.getElementById('name-input');
            currentUserName = nameInput ? nameInput.value.trim() : '';
        }
        ...
        if (roomCodeDisplay) {
            ...
            console.warn('roomCodeDisplay element not found');
        }
        ...
        if (roomDiv) roomDiv.style.display = 'flex';

        socket.emit('join-room', id, socket.id, currentUserName, userRole);
...
socket.on('user-connected', (userId, userName) => {
    console.log(`%c[Signaling] User connected: ${userId} (${userName})`, 'color: green; font-weight: bold');

    if (statusMsg) statusMsg.innerText = 'Connected';
    if (remoteWrapper) remoteWrapper.style.display = 'flex';
    if (videoGrid) videoGrid.classList.add('two-users');

    // Store connected user's name if available
    if (userName) {
        connectedStudentName = userName;
        console.log('[Signaling] Connected student name stored:', connectedStudentName);
    }

    // Send my name to the new user (so they know who I am)
    const myName = sessionStorage.getItem('userName');
    if (myName) {
        console.log('[Signaling] Sending my name to peer:', myName);
        socket.emit('send-name', myName);
    }

    console.log('[Signaling] Initiating offer creation...');
    createOffer();
});
...
socket.on('user-disconnected', () => {
    ...
    if (statusMsg) statusMsg.innerText = 'Waiting for someone to join...';
...
socket.on('offer', async (offer) => {
    try {
        console.log('%c[Signaling] Received OFFER', 'color: blue; font-weight: bold');
        if (!peerConnection) {
            console.log('[WebRTC] Creating PeerConnection for incoming offer');
        ...
        console.error('%c[WebRTC Error] Error handling offer:', 'color: red', error);
    }
});

socket.on('answer', async (answer) => {
    try {
        console.log('%c[Signaling] Received ANSWER', 'color: purple; font-weight: bold');
        console.log('[WebRTC] Setting Remote Description (Answer)...');
        await peerConnection.setRemoteDescription(new RTCSessionDescription(answer));
        console.log('[WebRTC] Remote Description set successfully. Connection should be establishing...');
    } catch (error) {
        console.error('%c[WebRTC Error] Error handling answer:', 'color: red', error);
    }
});
...
socket.on('ice-candidate', async (candidate) => {
    try {
        if (peerConnection) {
            ...
            console.warn('[WebRTC Warning] Received ICE candidate but PeerConnection is null');
        }
    } catch (error) {
        console.error('%c[WebRTC Error] Error adding ICE candidate:', 'color: red', error);
    }
});

function createPeerConnection() {
    console.log('%c[WebRTC] Creating new RTCPeerConnection', 'color: orange; font-weight: bold', config);
    peerConnection = new RTCPeerConnection(config);

    peerConnection.oniceconnectionstatechange = () => {
        console.log(`%c[WebRTC State] ICE Connection State: ${peerConnection.iceConnectionState}`, 'color: blue');
        if (peerConnection.iceConnectionState === 'failed' || peerConnection.iceConnectionState === 'disconnected') {
            console.warn('%c[WebRTC Warning] ICE connection failed or disconnected', 'color: red');
        }
        if (peerConnection.iceConnectionState === 'connected') {
            console.log('%c[WebRTC Success] ICE Connection Established!', 'color: green; font-weight: bold; font-size: 1.2em');
        }
    };

    peerConnection.onsignalingstatechange = () => {
        console.log(`[WebRTC State] Signaling State: ${peerConnection.signalingState}`);
    };

    peerConnection.onconnectionstatechange = () => {
        console.log(`[WebRTC State] Connection State: ${peerConnection.connectionState}`);
    };
    ...
    peerConnection.onicecandidate = (event) => {
        if (event.candidate) {
            ...
            console.log('[WebRTC] Finished generating ICE candidates');
        }
    };

    peerConnection.ontrack = (event) => {
        console.log(`%c[WebRTC] Track received: ${event.track.kind}`, 'color: green; font-weight: bold');
        ...
        remoteVideo.play()
            ...
            .catch(e => console.error('%c[WebRTC Error] Error auto-playing remote video:', 'color: red', e));

        // Connect remote audio to mixer
        if (event.track.kind === 'audio') {
            console.log('[Audio] Connecting remote audio to mixer');
    ...
    if (localStream) {
        ...
        console.warn('%c[WebRTC Warning] createPeerConnection called before localStream initialized', 'color: red');
    }
}

async function createOffer() {
    console.log('[WebRTC] createOffer called');
    if (!peerConnection) {  // Only create if doesn't exist
        console.log('[WebRTC] Creating PeerConnection for offer');
        createPeerConnection();
    }
    try {
        ...
        console.error('%c[WebRTC Error] Error creating offer:', 'color: red', err);
    }
}
...
// Setup Video Layout based on Role
function setupVideoLayout() {
    const instructorCol = document.getElementById('instructor-column');
    const studentCol = document.getElementById('student-column');
    const localWrapper = document.getElementById('local-wrapper');
    const remoteWrapper = document.getElementById('remote-wrapper');

    if (userRole === 'admin') {
        // Admin is Instructor
        if (instructorCol && localWrapper) instructorCol.appendChild(localWrapper);
        if (studentCol && remoteWrapper) studentCol.appendChild(remoteWrapper);
    } else {
        // Student
        if (studentCol && localWrapper) studentCol.appendChild(localWrapper);
        if (instructorCol && remoteWrapper) instructorCol.appendChild(remoteWrapper);
    }
}
...
function startLiveTranscription() {
    ...
    recognition.onresult = (event) => {
        ...
        if (finalTranscript) {
            ...
            appendTranscript(userRole, finalTranscript);
        }
    };
...
function appendTranscript(role, text) {
    // Determine which box to append to based on role
...
socket.on('subtitle', (data) => {
    ...
    const role = typeof data === 'object' ? data.role : 'student'; // Default to student if unknown
